  ...
  async function testConnection(rawUrl = form.url) {
    ...
    try {
      ...
      setTestResult({
        ...
        message: data?.message || `Connection failed: backend returned ${res.status}`,
      })
    } catch (error) {
      setTestResult({
        ok: false,
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Add each agent and its Mission Control gateway URL. This powers cross-agent cron and task views.
          </p>
        </div>
