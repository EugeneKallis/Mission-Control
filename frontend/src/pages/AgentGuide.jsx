        ...
        <div className='grid grid-cols-1 xl:grid-cols-[360px,1fr] gap-6'>
          <aside className='bg-slate-900/50 border border-slate-800 rounded-lg p-4 space-y-4 h-fit'>
            ...
            <div>
              <label className='text-xs text-slate-500 block mb-1'>
                Discord webhook <span className='text-slate-600'>(optional — notifies on task done)</span>
              </label>
              <input
            ...
            <div className='rounded border border-slate-700 bg-slate-800/40 p-3 text-xs text-slate-300 space-y-2'>
              <div>
                <span className='text-slate-500'>Schedule:</span> <span className='font-mono text-blue-300'>{scheduleLabel}</span>
              </div>
              <div>
                <span className='text-slate-500'>Job name:</span> <span className='font-mono text-violet-300'>{jobName}</span>
              </div>
              <div>
                <span className='text-slate-500'>Board state:</span> <span className='font-mono break-all'>{stateUrl}</span>
              </div>
              <div>
                <span className='text-slate-500'>Task updates:</span> <span className='font-mono break-all'>{updateTodoUrl}</span>
              </div>
              {targetAgent && (
                <div>
                  <span className='text-slate-500'>Agent gateway:</span> <span className='font-mono break-all'>{targetAgent.url}</span>
                </div>
              )}
            </div>
          </aside>
          ...
          <section className='space-y-6'>
            <div className='bg-slate-900/50 border border-slate-800 rounded-lg p-4'>
              ...
              <pre className='text-xs text-slate-300 bg-slate-950/80 rounded p-4 whitespace-pre-wrap overflow-x-auto max-h-[540px]'>
                {bootstrapPrompt || "Add an agent in Settings first."}
              </pre>
            </div>
            ...
            <div className='bg-slate-900/50 border border-slate-800 rounded-lg p-4'>
              ...
              <pre className='text-xs text-slate-300 bg-slate-950/80 rounded p-4 whitespace-pre-wrap overflow-x-auto max-h-[420px]'>
                {workerPrompt || "No target agent selected."}
              </pre>
            </div>
