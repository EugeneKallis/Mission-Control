...
function ArrInstanceCard({
  ...
  return (
    <div
      ...
      style={{ background: "#131313", border: "1px solid rgba(59, 75, 63, 0.25)" }}
    >
      <h3 className="text-sm font-semibold text-[#E5E2E1] mb-3">{name}</h3>

      <label className="block text-xs text-[#849587] mb-1">URL</label>
      <input
        type="url"
        className="w-full bg-[#201F1F] border border-[#3B4B3F] rounded px-2 py-1.5 text-xs font-mono text-[#E5E2E1] outline-none focus:border-[#618B6B] transition-colors mb-1"
        placeholder={defaultUrl}
        value={url}
        onChange={(e) => onFieldChange("url", e.target.value)}
      />
      {!url && (
        <p className="text-[10px] text-[#618B6B] mb-2">
          Default: {defaultUrl}
        </p>
      )}

      <label className="block text-xs text-[#849587] mb-1">API Key</label>
      <input
        type="password"
        className="w-full bg-[#201F1F] border border-[#3B4B3F] rounded px-2 py-1.5 text-xs font-mono text-[#E5E2E1] outline-none focus:border-[#618B6B] transition-colors"
...
export function ArrConfigSection({ values, onChange }: ArrConfigSectionProps) {
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold text-[#E5E2E1] mb-4">
        Arr Instances
      </h2>
      <p className="text-xs text-[#849587] mb-4">
        ...
        (<code className="text-[#618B6B]">ARR__&lt;NAME&gt;__URL</code>,{" "}
        <code className="text-[#618B6B]">ARR__&lt;NAME&gt;__API_KEY</code>)
        take precedence over stored values.
      </p>

      {/* Bulk import */}
      <label className={`${labelClass} text-[#849587]`}>
        Bulk Import
      </label>
      <p className="text-xs text-[#849587] mb-2">
