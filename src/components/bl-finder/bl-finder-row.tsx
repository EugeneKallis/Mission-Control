...
export function BlFinderRow({
  ...
  return (
    <div
      ...
      style={{
        background: row.status === "checking" ? "rgba(86, 255, 167, 0.04)" : "var(--color-surface-container)",
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[10px] mt-0.5" style={{ color: "var(--color-on-surface-variant)" }}>
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, { bg: string; fg: string; icon: string }> = {
    pending:   { bg: "rgba(132, 149, 135, 0.15)", fg: "var(--color-on-surface-variant)", icon: "schedule" },
    checking:  { bg: "rgba(86, 255, 167, 0.15)",   fg: "var(--color-primary-fixed)", icon: "progress_activity" },
    ok:        { bg: "rgba(86, 255, 167, 0.15)",   fg: "var(--color-primary-fixed)", icon: "check_circle" },
